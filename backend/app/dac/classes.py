...
class Municipio:
    ...
    def taxa_reprovacao(self, ano: int) -> float:
        rep = self.total_reprovados(ano)
        base = self.total_aprovados(ano) + rep + self.total_abandono(ano)
        return round((rep / base) * 100, 2) if base > 0 else 0.0
    ...
    def resumo_ano(self, ano: int) -> dict:
        return {
            ...
            "taxa_reprovacao": self.taxa_reprovacao(ano),
    ...
    def resumo_para_llm(self, municipio: Optional[str] = None, ano: Optional[int] = None) -> str:
        ...
        if municipio and ano:
            ...
            if m:
                ...
                linhas.append(f"- Aprovados: {r['aprovados']:,} (taxa de aprovação: {r['taxa_aprovacao']}%)")
                linhas.append(f"- Reprovados: {r['reprovados']:,} (taxa de reprovação: {r['taxa_reprovacao']}%)")
                linhas.append(f"- Abandono: {r['abandono']:,} (taxa de abandono: {r['taxa_abandono']}%)")
        elif municipio:
            m = self.municipios.get(municipio)
            if m:
            ...
            for r in municipios_ano[:20]:
                linhas.append(
                    ...
                    f"Aprovação: {r['taxa_aprovacao']}% | "
                    f"Reprovação: {r['taxa_reprovacao']}% | "
